...
class CartItemResponse(BaseModel):
    ...
    unit_price: Decimal = Field(default=Decimal("0.00"), description="Цена за единицу товара")
    subtotal: Decimal = Field(default=Decimal("0.00"), description="Подытог по позиции (цена * кол-во)")
    is_available: bool = Field(default=True, description="Доступен ли товар на складе в запрашиваемом количестве")
...
class CartResponse(BaseModel):
    ...
    total_items: int
    total_amount: Decimal = Field(default=Decimal("0.00"), description="Общая стоимость товаров в корзине")
    has_unavailable_items: bool = Field(default=False, description="Есть ли в корзине недоступные товары")
